...
def plot(sampleset,channel,parallel=True,tag="",extratext="",outdir="plots",era="",
  ...
  for wp, wpcut in wps:
    ...
    for i, ptlow in enumerate(ptList):
      ...
      for eta in etas:
        name_ = "%s_eta%s"%(name,eta)
        tit_  = "%s, eta%s"%(tit,eta)
        if eta =="Barrel":
          cut_ = "%s && abs(JetEta) <= 1.5"%(cut)
        else: # eta == "Endcap"
          cut_ = "%s && abs(JetEta) > 1.5 && abs(JetEta) <= 2.4"%(cut) 
        selections.append(Sel(name_,tit_,cut_,only=["JetPt"]))
