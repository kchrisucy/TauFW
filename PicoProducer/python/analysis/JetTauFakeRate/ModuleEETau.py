...
class ModuleEETau(Module):
  ...
  def beginJob(self):
    ...
    self.TauDz              = np.zeros(1,dtype='f')      
    self.TauGenPartFlav     = np.zeros(1,dtype='i')
    ...
    self.tree.Branch("TauDz"          ,  self.TauDz         , "TauDz/F"          )
    self.tree.Branch("TauGenPartFlav" ,  self.TauGenPartFlav, "TauGenPartFlav/I" )
  ...
  def analyze(self, event):
    ...
    for muon in Collection(event,'Muon'):
      ...
      if any(muon.DeltaR(muon)<0.3 for electron in electrons): continue ## remove overlap with selected muons as well !?! dR<0.3 optimal???
    ...
    TauDz             = tau.dz
    if self.ismc :
      TauGenPartFlav    = tau.genPartFlav
    else :
      TauGenPartFlav    = -1
      #print tau.idDeepTau2017v2p1VSe, tau.idDeepTau2017v2p1VSmu
    JetN              = len(jets)
    ...
    self.TauDz[0]              = TauDz         
    self.TauGenPartFlav[0]     = TauGenPartFlav
    self.JetN[0]               = JetN            
    self.MET[0]                = MET
    self.HT[0]                 = HT 
    self.LT[0]                 = LT
